...
<head>
  ...
  <title>Configuraciones | Permisos</title>
  <?php
  ...
  <div class="content-wrapper">
    ...
    $sqlPermiso = ("SELECT * FROM permiso WHERE estatus = 1 ORDER BY Id_permiso ASC");
    ...
    <section class="content-header">
      <h1>
        Permisos (<?php echo $total_permiso; ?>)
      </h1>
      <ol class="breadcrumb">
    ...
    <section class="content">
      <div style="padding-bottom: 10px;">
        ...
        <?php if (in_array('Ver papelera de permisos', $_SESSION["permisos"])) : ?>
          <a href="papelera/cfg_permisos_papelera_listado.php" class="btn-sm btn-primary pull-right" style="background-color: gray;"> Papelera </a>
        <?php endif; ?>
        <p class="pull-right" style="width:5px;"></p>
        <?php if (in_array('Crear Permisos', $_SESSION["permisos"])) : ?>
          <a href="cfg_permisos_agregar.php" class="btn-sm btn-success pull-right"> Añadir Un Nuevo Permiso </a>
        <?php endif; ?>
        ...
        <table class="table table-sm table-hover mt-4" width="100%" height="20" id="t_user">
          ...
          <tbody class="tbody" width="100%" style="font-size: 12px;">
            <tr>
              ...
              $sql = "SELECT * FROM permiso $donde ORDER BY Id_permiso ASC LIMIT $inicio, $registros_por_pagina";
              ...
              <?php if (in_array('Gestionar acciones de permisos', $_SESSION["permisos"])) : ?>
                <td>
                  <?php if (in_array('Editar Permisos', $_SESSION["permisos"])) : ?>
                    <a href="cfg_permisos_editar.php?Id=<?php echo $row['Id_permiso'] ?>" class="btn-sm btn-warning" title="Editar"><img src="../../recursos/imagenes/iconos/editar.png" style="width:15px; height:15px;"></a>
                  <?php endif; ?>
                  <?php if (in_array('Desactivar Permisos', $_SESSION["permisos"])) : ?>
                    <a href="#" data-id="<?php echo $row['Id_permiso'] ?>" title="Desactivar" class="btn-sm btn-danger btn-desactivar"><img src="../../recursos/imagenes/iconos/Delete.png" style="width:15px; height:15px;"></a>
                  <?php endif; ?>
                </td>
              <?php endif; ?>
            </tr>
            <?php }
              ...
              echo "<tr><td colspan='6'>No se encontraron permisos registrados.</td></tr>";
            } ?>
          </tbody>
        </table>
      </div>
    ...
    <div class="modal" id="DesactivarPermiso" tabindex="-1" role="dialog" aria-labelledby="DesactivarPermisoLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" style="background-color: #dc3545; color: white;">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="DesactivarPermisoLabel">Confirmar Desactivacion</h4>
          </div>
          <div class="modal-body">
            <p>¿Está seguro de que desea desactivar este permiso? Esta acción solo se puede revertir en la papelera.</p>
          </div>
          <div class="modal-footer">
    ...
    <script>
      $(document).ready(function() {
        ...
        $('#DesactivarPermiso .close, #DesactivarPermiso .btn-second').on('click', function() {
          closeCustomModal($('#DesactivarPermiso'));
        });

        $(document).on('click', '.btn-desactivar', function(e) {
          e.preventDefault();
          var IdPermiso = $(this).data('id');
          var urlDesactivar = "../../cfg/desactivar/desactivar_permiso.php?Id=" + IdPermiso;
          $('#desactivar').attr('href', urlDesactivar);
          $('#DesactivarPermiso').modal('show');
        })
