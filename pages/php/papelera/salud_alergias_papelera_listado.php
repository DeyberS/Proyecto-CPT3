...
<head>
  ...
  <div class="content-wrapper">
    ...
    $sqlAlergia = ("SELECT * FROM alergias_conocidas WHERE estatus = 0 ORDER BY Id_alergias_conocidas ASC");
    ...
    <section class="content-header">
      <h1>
        Alergias Inactivas (<?php echo $total_alergia; ?>)
      </h1>
      <ol class="breadcrumb">
    ...
    <section class="content">
      <div style="padding-bottom: 10px;">
        <a href="../salud_alergias_listado.php" class="btn-sm btn-primary pull-right"> Regresar al Listado </a>
        ...
        <table class="table table-sm table-hover mt-4" width="100%" height="20" id="t_user">
          ...
          <tbody class="tbody" width="100%" style="font-size: 12px;">
            <tr>
              ...
              // 3. Construir el filtro base (Alergias inactivas)
              $donde = "WHERE estatus = 0";
              if ($busqueda != '') {
                $donde .= " AND (nombre_alergia LIKE '%$busqueda%')";
              }

              // 4. Conteo de registros filtrados para recalcular la paginación
            ...
            <tr>
              ...
              <?php if (in_array('Gestionar acciones de alergias', $_SESSION["permisos"])) : ?>
                <td>
                  <?php if (in_array('Reactivar Alergias', $_SESSION["permisos"])) : ?>
                    <a href="#" data-id="<?php echo $row['Id_alergias_conocidas'] ?>" class="btn-sm btn-success btn-reactivar" title="Reactivar"><img src="../../../recursos/imagenes/iconos/reactivar.png" style="width:15px; height:15px;"></a>
                  <?php endif; ?>
                  <?php if (in_array('Eliminar Alergias', $_SESSION["permisos"])) : ?>
                    <a href="#" data-id="<?php echo $row['Id_alergias_conocidas'] ?>" class="btn-sm btn-danger btn-eliminar" title="Eliminar"><img src="../../../recursos/imagenes/iconos/Delete.png" style="width:15px; height:15px;"></a>
                  <?php endif; ?>
                </td>
            </tr>
          <?php endif; ?>
    ...
    <div class="modal" id="ReactivarAlergia" tabindex="-1" role="dialog" aria-labelledby="ReactivarAlergiaLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" style="background-color: #dc3545; color: white;">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="ReactivarAlergiaLabel">Confirmar Desactivacion</h4>
          </div>
          <div class="modal-body">
            <p>¿Está seguro de que desea reactivar esta alergia</p>
          </div>
          <div class="modal-footer">
    ...
    <div class="modal" id="EliminarAlergia" tabindex="-1" role="dialog" aria-labelledby="EliminarAlergiaLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" style="background-color: #dc3545; color: white;">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="EliminarAlergiaLabel">Confirmar Desactivacion</h4>
          </div>
          <div class="modal-body">
            <p>¿Está seguro de que desea eliminar esta alergia? Esta acción no se puede deshacer.</p>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-second" data-dismiss="modal">Cancelar</button>
    ...
    <script>
      $(document).ready(function() {
        ...
        $('#ReactivarAlergia .close, #ReactivarAlergia .btn-second').on('click', function() {
          closeCustomModal($('#ReactivarAlergia'));
        });

        $('#EliminarAlergia .close, #EliminarAlergia .btn-second').on('click', function() {
          closeCustomModal($('#EliminarAlergia'));
        });

        $(document).on('click', '.btn-reactivar', function(e) {
          e.preventDefault();
          var IdAlergia = $(this).data('id');
          var urlReactivar = "../../../cfg/reactivar/reactivar_alergia.php?Id=" + IdAlergia;
          $('#reactivar').attr('href', urlReactivar);
          $('#ReactivarAlergia').modal('show');
        })

        $(document).on('click', '.btn-eliminar', function(e) {
          e.preventDefault();
          var IdAlergia = $(this).data('id');
          var urlEliminar = "../../../cfg/eliminar/eliminar_alergia.php?Id=" + IdAlergia;
          $('#eliminar').attr('href', urlEliminar);
          $('#EliminarAlergia').modal('show');
        })
