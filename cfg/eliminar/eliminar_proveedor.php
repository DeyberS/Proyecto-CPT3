...
if (isset($_GET['Id'])) {
    ...
    try {
        $check = $conexion->prepare("SELECT COUNT(*) FROM lotes_medicamentos WHERE Id_proveedor = ?");
        $check->bind_param("i", $id);
        $check->execute();
        $res = $check->get_result()->fetch_row();
        
        if ($res[0] > 0) {
            throw new Exception("No se puede eliminar el proveedor porque tiene lotes asignados.");
        }
        ...
        $conexion->commit();
        $_SESSION['mensaje_user_exito'] = "✅ Éxito: El proveedor fue eliminado correctamente.";
