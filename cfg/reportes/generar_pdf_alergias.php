...
switch ($tipo) {
    case 'activas':
        ...
        $subtitulo = "Listado de alergias vigentes para diagnóstico";
        $sql = "SELECT * FROM alergias_conocidas WHERE estatus = 1 ORDER BY nombre_alergia ASC";
        break;
    case 'inactivas':
        $titulo = "CATÁLOGO DE ALERGIAS INACTIVAS";
        $subtitulo = "Listado de alergias desincorporadas del sistema";
        $sql = "SELECT * FROM alergias_conocidas WHERE estatus = 0 ORDER BY nombre_alergia ASC";
        break;
    default:
        $titulo = "CATÁLOGO MAESTRO DE ALERGIAS";
        $subtitulo = "Registro general de alergias conocidas";
...
// 2. CLASE EXTENDIDA PARA MEMBRETE CLÍNICO
class MYPDF extends TCPDF {
    public function Header() {
        $this->SetY(15); 
        $this->SetFont('helvetica', 'B', 14);
        $this->SetTextColor(44, 62, 80); // Azul Pizarra (Clínico)
...
<style>
    ...
    th { background-color: #34495E; color: white; font-weight: bold; font-size: 7pt; text-align: center; border: 1px solid #cccccc; }
...
<table cellpadding="6">
    <thead>
        <tr>
            <th width="10%">Nº</th>
            <th width="70%">Descripción de la Alergia</th>
...
if(mysqli_num_rows($resultado) > 0){
    while ($row = mysqli_fetch_assoc($resultado)) {
        ...
        $html .= '<tr class="'.$clase.'">
                    ...
                    <td width="70%" style="text-align:left;">'.mb_strtoupper($row['nombre_alergia']).'</td>
...
$pdf->Output('Catalogo_Alergias.pdf', 'I');
?>