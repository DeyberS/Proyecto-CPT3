                ...
                <div class="box-body">
                  <form action="../../cfg/editar/editar_lote.php" id="formularioLote" class="form-group" method="POST" novalidate>
                    ...
                    <div class="col-sm-4 form-group" id="group_vencimiento">
                      <p>F. Vencimiento (*):</p>
                      <input type="date" class="form-control" id="fecha_vencimiento" name="fecha_vencimiento" min="<?php echo date('Y-m-d'); ?>" value="<?php echo $row['fecha_vencimiento'] ?>" required>
                    </div>
                    ...
                    <div class="col-sm-4">
                      ...
                      <select name="estado_lote" id="estado_lote" class="form-control">
                        ...
                        <option value="Vencido" <?php echo ($row['estado_lote'] == 'Vencido' ? 'selected' : ''); ?>>Vencido</option>
                        <option value="Retirado" <?php echo ($row['estado_lote'] == 'Retirado' ? 'selected' : ''); ?>>Retirado</option>
                        <option value="Dañado" <?php echo ($row['estado_lote'] == 'Dañado' ? 'selected' : ''); ?>>Dañado</option>
                      </select>
                    </div>
                      ...
                      <select id="medicamento" name="medicamento" class="form-control" required>
                        ...
                        if ($resultado_medicamentos && $resultado_medicamentos->num_rows > 0) {
                          while ($row_med = $resultado_medicamentos->fetch_assoc()) {
                            $selected = ($row_med['Id'] == $row['Id_medicamento']) ? 'selected' : '';
                            echo "<option value='" . $row_med['id_desc'] . "' " . $selected . ">" . $row_med['nombre_medicamento'] . "</option>";
                          }
                        }
                        ?>
                      </select>
    ...
    <div class="modal" id="modalGuardar" tabindex="-1" role="dialog">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>¿Está seguro de que desea actualizar la informacion del lote?</p>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
    ...
    <script>
      $(document).ready(function() {
        ...
        $('#fecha_fabricacion, #fecha_vencimiento').on('keydown', function(e) {
          e.preventDefault(); // Solo calendarios
        });
        ...
        function verificarLoteYMostrarModal() {
          ...
          const id_lote = $('input[name="Id"]').val(); // ID del lote que estamos editando
          ...
          $.ajax({
            // ⚠️ Asegúrate de que esta URL apunte correctamente a tu script PHP ⚠️
            // Si el editar está en la misma carpeta que agregar, pon la misma ruta
            ...
            data: {
              ...
              id_lote: id_lote // Excluimos este ID
            },
            success: function(response) {
              ...
              if (response.existe_lote) {
                ...
                mostrarAviso('🛑 Error de Duplicidad:<br>Ya existe OTRO lote con el nombre <b>' + nombre + '</b> asignado a este mismo medicamento.');
        ...
        formulario.on('submit', function(e) {
          ...
          if (errores.length > 0) {
            ...
            // Llama a AJAX en lugar de abrir el modal directo
        ...
        $('#confirmarGuardar').on('click', function() {
          ...
          formulario[0].submit(); // Envío directo del DOM bypassando jQuery
        });
