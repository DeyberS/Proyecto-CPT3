...
<head>
  ...
  <div class="content-wrapper">
    ...
    $sqlRol = ("SELECT * FROM rol WHERE estatus = 1 ORDER BY Id_rol ASC");
    ...
    <section class="content">
      <div style="padding-bottom: 10px;">
        ...
        <?php if (in_array('Ver papelera de roles', $_SESSION["permisos"])) : ?>
          <a href="papelera/cfg_roles_papelera_listado.php" class="btn-sm btn-primary pull-right" style="background-color: gray;"> Papelera </a>
        <?php endif; ?>
        <p class="pull-right" style="width:5px;"></p>
        <?php if (in_array('Crear Roles', $_SESSION["permisos"])) : ?>
          <a href="cfg_roles_agregar.php" class="btn-sm btn-success pull-right"><i class="fa fa-user-plus"></i> Añadir Un Nuevo Rol </a>
        <?php endif; ?>
        ...
        <table class="table table-sm table-hover mt-4" width="100%" height="20" id="t_user">
          ...
          <tbody class="tbody" width="100%" style="font-size: 12px;">
            <tr>
              ...
              $total_roles = $fila_conteo['total']; // Usar esta variable para el total de páginas

              $registros_por_pagina = 14;
              $pagina_actual = isset($_GET['pagina']) ? (int)$_GET['pagina'] : 1;
              $inicio = ($pagina_actual - 1) * $registros_por_pagina;
              $total_paginas = ceil($total_roles / $registros_por_pagina);
              ...
              <?php if (in_array('Gestionar acciones de roles', $_SESSION["permisos"])) : ?>
                <td>
                  <?php if (in_array('Editar Roles', $_SESSION["permisos"])) : ?>
                    <a href="cfg_roles_editar.php?Id=<?php echo $row['Id'] ?>" class="btn-sm btn-warning" title="Editar"><img src="../../recursos/imagenes/iconos/editar.png" style="width:15px; height:15px;"></a>
                  <?php endif; ?>
    ...
    <div class="modal" id="DesactivarRol" tabindex="-1" role="dialog" aria-labelledby="DesactivarRolLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" style="background-color: #dc3545; color: white;">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="DesactivarRolLabel">Confirmar Desactivacion</h4>
          </div>
          <div class="modal-body">
            <p>¿Está seguro de que desea desactivar este rol? Esta acción solo se puede revertir en la papelera.</p>
          </div>
          <div class="modal-footer">
    ...
    <script>
      $(document).ready(function() {
        ...
        $('#DesactivarRol .close, #DesactivarRol .btn-second').on('click', function() {
          closeCustomModal($('#DesactivarRol'));
        });

        $(document).on('click', '.btn-desactivar', function(e) {
          e.preventDefault();
          var IdRol = $(this).data('id');
          var urlDesactivar = "../../cfg/desactivar/desactivar_rol.php?Id=" + IdRol;
          $('#desactivar').attr('href', urlDesactivar);
          $('#DesactivarRol').modal('show');
        })
