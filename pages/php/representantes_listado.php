...
<head>
  ...
  <title>Representantes | Listado</title>
  <?php
  ...
  <div class="content-wrapper">
    ...
    $sqlRepresentantes = ("SELECT COUNT(p.id) AS total_representantes FROM persona p 
      ...
      WHERE r.Id_rol = 5 AND TIMESTAMPDIFF(YEAR, p.fecha_nacimiento, CURDATE()) >= 18 AND p.estatus = 1");
    ...
    <section class="content-header">
      <h1>
        Representantes (<?php echo $total_representantes; ?>)
      </h1>
      <ol class="breadcrumb">
    ...
    <section class="content">
      <div style="padding-bottom: 10px;">
        <?php if (in_array('Ver papelera de representantes', $_SESSION["permisos"])) : ?>
          <a href="papelera/representantes_papelera_listado.php" class="btn-sm btn-primary pull-right" style="background-color:gray;"> Papelera </a>
        <?php endif; ?>
        <p class="pull-right" style="width:5px;"></p>
        <?php if (in_array('Generar Reportes de Representantes', $_SESSION["permisos"])) : ?>
          <a href="#" class="btn-sm btn-info pull-right reporte"><i class="fa fa-book"></i> Generar Reporte </a>
        <?php endif; ?>
        <p class="pull-right" style="width:5px;"></p>
        <?php if (in_array('Crear Representantes', $_SESSION["permisos"])) : ?>
          <a href="representantes_agregar.php" class="btn-sm btn-success pull-right"><i class="fa fa-user-plus"></i> Añadir Un Nuevo Representante </a>
        <?php endif; ?>
      ...
      <div id="contenedorTabla">
        <table class="table table-sm table-hover mt-4" width="100%" height="20" id="t_user">
          ...
          <tbody class="tbody" width="100%" style="font-size: 12px;">
            ...
            // 2. Definir el filtro base para representantes activos (Rol 4)
            $donde = "WHERE r.Id_rol = 5 AND p.estatus = 1 AND TIMESTAMPDIFF(YEAR, p.fecha_nacimiento, CURDATE()) >= 18";
            ...
            $sqlConteo = "SELECT COUNT(DISTINCT p.id) as total 
                      FROM persona p 
                      JOIN detalle_persona_rol dpr ON p.id = dpr.Id_persona 
                      JOIN rol r ON dpr.Id_rol = r.Id_rol 
                      $donde";
            $queryConteo = mysqli_query($conexion, $sqlConteo);
            $total_representantes = mysqli_fetch_assoc($queryConteo)['total'];
            $total_paginas = ceil($total_representantes / $registros_por_pagina);
                ...
                <tr>
                  ...
                  <?php if (in_array('Gestionar acciones de representantes', $_SESSION["permisos"])) : ?>
                    <td>
                      ...
                      <?php endif; ?>
                      <?php if (in_array('Desactivar Representantes', $_SESSION["permisos"])) : ?>
    ...
    <div class="modal" id="DesactivarRepresentante" tabindex="-1" role="dialog" aria-labelledby="DesactivarRepresentanteLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" style="background-color: #dc3545; color: white;">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="DesactivarRepresentanteLabel">Confirmar Desactivacion</h4>
          </div>
          <div class="modal-body">
    ...
    <div class="modal" id="ModalReporteRepresentante" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" style="background-color: #337ab7; color: white;">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="myModalLabel"><i class="fa fa-file-pdf-o"></i> Reportes de Representantes / Contactos</h4>
          </div>
          <div class="modal-body">
            <div class="form-group">
              <label>Seleccione el filtro del reporte:</label>
              <select class="form-control" id="tipo_reporte_representante">
                <option value="todos">Todos los Representantes</option>
                <option value="activos">Solo Representantes Activos</option>
                <option value="inactivos">Solo Representantes Inactivos</option>
              </select>
            </div>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-second" data-dismiss="modal">Cerrar</button>
            <button type="button" class="btn btn-primary" id="btnEjecutarReporteRepresentante">Generar PDF</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <script>
      $(document).ready(function() {
        ...
        $('#DesactivarRepresentante .close, #DesactivarRepresentante .btn-second').on('click', function() {
          closeCustomModal($('#DesactivarRepresentante'));
        });

        $('.reporte').on('click', function(e) {
          e.preventDefault();
          $('#ModalReporteRepresentante').modal('show');
        });

        // Cerrar modal usando tu función closeCustomModal
        $('#ModalReporteRepresentante .close, #ModalReporteRepresentante .btn-second').on('click', function() {
          closeCustomModal($('#ModalReporteRepresentante'));
        });

        // Ejecutar la generación del reporte
        $('#btnEjecutarReporteRepresentante').on('click', function() {
          var tipo = $('#tipo_reporte_representante').val();
          window.open('../../cfg/reportes/generar_pdf_representantes.php?tipo=' + tipo, '_blank');
          $('#ModalReporteRepresentante').modal('hide');
        });

        $(document).on('click', '.btn-desactivar', function(e) {
          e.preventDefault();
          var IdRepresentante = $(this).data('id');
          var urlDesactivar = "../../cfg/desactivar/desactivar_representante.php?Id=" + IdRepresentante;
          $('#desactivar').attr('href', urlDesactivar);
          $('#DesactivarRepresentante').modal('show');
        })
