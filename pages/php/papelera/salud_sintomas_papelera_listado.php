...
<head>
  ...
  <div class="content-wrapper">
    ...
    $sqlSintoma = ("SELECT * FROM sintomas WHERE estatus = 0 ORDER BY Id_sintomas ASC");
    ...
    <section class="content">
      <div style="padding-bottom: 10px;">
        <a href="../salud_sintomas_listado.php" class="btn-sm btn-primary pull-right"> Regresar al Listado </a>
        ...
        <table class="table table-sm table-hover mt-4" width="100%" height="20" id="t_user">
          ...
          <tbody class="tbody" width="100%" style="font-size: 12px;">
            <tr>
              ...
              // 3. Definir el filtro base (Síntomas inactivos)
              $donde = "WHERE estatus = 0";
              if ($busqueda != '') {
                $donde .= " AND (nombre_sintoma LIKE '%$busqueda%')";
              }

              // 4. Conteo de registros filtrados para la paginación
              $sql_conteo = "SELECT COUNT(*) as total FROM sintomas $donde";
              $resultado_conteo = mysqli_query($conexion, $sql_conteo);
              $fila_conteo = mysqli_fetch_assoc($resultado_conteo);
              $total_permisos = $fila_conteo['total']; // Se mantiene el nombre de variable original del archivo
            ...
            <tr>
              ...
              <?php if (in_array('Gestionar acciones de sintomas', $_SESSION["permisos"])) : ?>
                <td>
                  <?php if (in_array('Reactivar Sintomas', $_SESSION["permisos"])) : ?>
                    <a href="#" data-id="<?php echo $row['Id_sintomas'] ?>" class="btn-sm btn-success btn-reactivar" title="Reactivar"><img src="../../../recursos/imagenes/iconos/reactivar.png" style="width:15px; height:15px;"></a>
                  <?php endif; ?>
                  <?php if (in_array('Eliminar Sintomas', $_SESSION["permisos"])) : ?>
                    <a href="#" data-id="<?php echo $row['Id_sintomas'] ?>" class="btn-sm btn-danger btn-eliminar" title="Eliminar"><img src="../../../recursos/imagenes/iconos/Delete.png" style="width:15px; height:15px;"></a>
                  <?php endif; ?>
                </td>
            </tr>
          <?php endif; ?>
    ...
    <div class="modal" id="ReactivarSintoma" tabindex="-1" role="dialog" aria-labelledby="ReactivarSintomaLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" style="background-color: #dc3545; color: white;">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="ReactivarSintomaLabel">Confirmar Desactivacion</h4>
          </div>
          <div class="modal-body">
            <p>¿Está seguro de que desea reactivar este sintoma</p>
          </div>
          <div class="modal-footer">
    ...
    <div class="modal" id="EliminarSintoma" tabindex="-1" role="dialog" aria-labelledby="EliminarSintomaLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header" style="background-color: #dc3545; color: white;">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="EliminarSintomaLabel">Confirmar Desactivacion</h4>
          </div>
          <div class="modal-body">
            <p>¿Está seguro de que desea eliminar este sintoma? Esta acción no se puede deshacer.</p>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-second" data-dismiss="modal">Cancelar</button>
    ...
    <script>
      $(document).ready(function() {
        ...
        $('#ReactivarSintoma .close, #ReactivarSintoma .btn-second').on('click', function() {
          closeCustomModal($('#ReactivarSintoma'));
        });

        $('#EliminarSintoma .close, #EliminarSintoma .btn-second').on('click', function() {
          closeCustomModal($('#EliminarSintoma'));
        });

        $(document).on('click', '.btn-reactivar', function(e) {
          e.preventDefault();
          var IdSintoma = $(this).data('id');
          var urlReactivar = "../../../cfg/reactivar/reactivar_sintoma.php?Id=" + IdSintoma;
          $('#reactivar').attr('href', urlReactivar);
          $('#ReactivarSintoma').modal('show');
        })

        $(document).on('click', '.btn-eliminar', function(e) {
          e.preventDefault();
          var IdSintoma = $(this).data('id');
          var urlEliminar = "../../../cfg/eliminar/eliminar_sintoma.php?Id=" + IdSintoma;
          $('#eliminar').attr('href', urlEliminar);
          $('#EliminarSintoma').modal('show');
        })
