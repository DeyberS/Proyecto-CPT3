                ...
                <section id="new" style="margin-bottom:11%;">
                  ...
                  <div class="col-sm-1" style="margin-top: 30px;">
                    <button type="button" class="form-control pull-right bt-sm btn-primary"  data-toggle="modal" data-target="#modalPatologias">
                      <img src="../../recursos/imagenes/iconos/agregar.png" height="20px" width="20px">
                    </button>
  ...
  <div class="modal" id="modalPatologias" tabindex="-1" role="dialog" aria-labelledby="modalPatologiasLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
          <button type="button" class="btn btn-success" id="guardarPatologias">Guardar</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="modal" id="modalAlergias" tabindex="-1" role="dialog" aria-labelledby="modalAlergiasLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
          <button type="button" class="btn btn-success" id="guardarAlergias">Guardar</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    async function validarCedulaFormato() {
      ...
      try {
        ...
        const existe = await verificarCedulaEnBD(tipo, cedulaVerificar);
        if (existe) {
          $(cedulaInput).addClass('input-error');
          mostrarAviso('🛑 Error de Cédula: El documento ' + tipo + '-' + valorCedula + ' ya se encuentra registrado en el sistema.');
          esValido = false;
        }
      } catch (error) {
        console.error(error);
...
// ... (El resto de funciones hasta validarCedulaRepFormato() se mantiene igual, excepto el cambio 2)
    ...
    $('#btnGuardarNuevaPatologiaAjax').click(function() {
      ...
      $.ajax({
          url: 'get/verificar_existencia_patologa.php',
          data: { nombre: nombre },
          success: function(res) { if(res.existe) mostrarAviso('Ya existe'); else guardar(); }
      });
    ...
    function actualizarDisplayPatologias() {
      ...
      if (nombres.length > 0) {
        ...
        if (nombres.length > LIMITE_DISPLAY) {
          ...
          htmlContent += `<span class="text" style="margin-left:5px; font-weight:bold;">... y ${restantes} más.</span>`;
        }
